---
title: "Philadelphia Night Time Economy Data Digest - 2024
...
TO DO - ARchive the Zoning stuff? Do a Klubkataster? Look at pct decline in gun incidents?
...
Analysis by Michael Fichman, Associate Professor of Practice, University of Pennsylvania Weitzman School of Design. 
...
Why is this happening? Wage tax returns are down across the board - and these kinds of industries externalize hospitality activity.
...
```{r model_amusement, warning = FALSE, message = FALSE, include=FALSE, eval = FALSE}
...
```{r projection_chart_amusement, warning = FALSE, message = FALSE, echo=FALSE, eval = FALSE}
amusement %>%
  ...
  ggplot()+
    ...
    labs(
        ...
        subtitle = "Adjusted for inflation to 2023 Dollars.\n Trend based on 2014-2/2020 regression controlling for year, month. \nSource: Philadelphia City Controller's Office")+
    plotTheme

```

```{r projection_chart_bar2, warning = FALSE, message = FALSE, echo=FALSE, eval = FALSE}
amusement %>%
  group_by(date, year, month) %>% 
  summarize(Actual = sum(total), 
            Pre.Pandemic.Trend = sum(pred)) %>% 
...
```{r amusememt_shortfall_pct, warning = FALSE, message = FALSE, echo=FALSE, eval = FALSE}
amusement %>%
    group_by(date, year, month) %>% 
    summarize(Actual = sum(total), 
              Pre.Pandemic.Trend = sum(pred), 
...
```{r loss_summary_amusement, echo=FALSE, message=FALSE, warning=FALSE, eval = FALSE}
...
There are currently 113 SAOLs active in Philly.
...
New Activations - there is some administration-related bump in 2004/05/06 where records were added to the database with that as the "activation" year
...
1. Firearm incidents are declining for the city as a whole after a peak in 2021.

2. Most incidents happen at night, between approximately 11PM and 1AM. 

3. Gun crime at night mostly happens away from establishments in commercial corridors (~PERCENTAGE), and mostly concentrated in neighborhoods with high volumes of incidents.
...
# URL encode the query
query <- URLencode("SELECT *, ST_Y(the_geom) AS lat, ST_X(the_geom) AS lng FROM incidents_part1_part2 WHERE dispatch_date_time >= '2024-01-01' AND dispatch_date_time < '2024-06-01'")
...
We examined a category of incidents recorded between January 1, 2017 and June 1, 2024 - incidents classified by the PPD as Criminal Homicide, Robbery with a Firearm, Aggravated Assault with a Firearm.
...
violentCrime %>%
    ...
    ylim(0, 500)+
    labs(title="Monthly Firearm Incidents By Type, 2017-2024",
...
As we saw in 2021/22, the corridors in the city with the most firearm incidents (aggravated assault, robbery, homicide) are the ones in areas with epidemic gun violence (e.g. Kensington and Allegheny, Broad and Susquehana, 52nd St.). This intensity is especially pronounced when you consider their size.
