...
Data on wage tax revenues obtained from Philadelphia Controller's Office, 3/8/2022.
...
Trend estimates - linear model based on year, quarter, and sector from January, 2014-February, 2020. Model explains >98% of variance in pre-pandemic, inflation-adjusted, tax receipts.
...
```{r projection_chart, warning = FALSE, message = FALSE}
wage_tax_revenue_new %>%
...
```{r projection_chart_bar, warning = FALSE, message = FALSE}
wage_tax_revenue_new %>%
  group_by(date, sector, year, month) %>% 
  summarize(Actual = sum(total), 
            Pre.Pandemic.Trend = sum(pred)) %>% 
...
Total tax losses (compared to pre-pandemic trend) since March, 2020, in millions (2021 dollars)

```{r loss_summary, message = FALSE, warning = FALSE}
...
wage_tax_revenue_new %>%
  ...
  group_by(sector) %>%
  summarize(total_loss_m = round(sum(loss)/1000000, digits = 2))
